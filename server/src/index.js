...
const GATES = {
  login: { amount: LOGIN_AMOUNT, label: 'Sign in' },
  'gate:vip': { amount: process.env.PORTAL_GATE_VIP_AMOUNT ?? '0.05', label: 'VIP Alpha Access' },
};
...
app.post('/auth/challenge', async (req, res) => {
  ...
  const challenge = createChallenge({ purpose, minAmountZats: zecToZats(gate.amount) });
...
app.post('/paylinks/:slug/challenge', async (req, res) => {
  const link = getPaylink(req.params.slug);
  if (!link) return res.status(404).json({ error: 'not found' });
